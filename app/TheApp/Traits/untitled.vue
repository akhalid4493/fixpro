...
trait PreviewTrait
{
    ...
    static public function technicalShift($users,$preview)
    {
        ...
        foreach ($data as $shift) {
            ...
            $data =self::shifts($shift,$preview);

            if ($data['startShift'] < $preview->time && $preview->time < $data['endShift']) {
    ...
    static public function checkAvilable($date,$service_id,$province_id)
    {
        ...
        $usersId = self::technicalShift($users->get(),$preview);

        return User::whereIn('id',$usersId)->count();
    }
}